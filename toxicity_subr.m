...
for i = 1:1
    ...
    [row,column] = size(S_HighDensity_ReducedData(i).XX);
    S_HighDensity_ReducedData(i).XdivPNEC = zeros(row,column);
    
    %Insert compounds with available data AND available PNEC into new
    %matrix
    for r = 1 : row
        row_notnan = ~isnan(S_HighDensity_ReducedData(i).XX(r,:));
        row_acceptable_for_PNEC = double(row_notnan).*double(S_HighDensity_ReducedData(i).avail_compounds)';
        if(~isempty(S_HighDensity_ReducedData(i).XX(r,logical(row_acceptable_for_PNEC))))
            %Divide compound concentration by PNEC value
            S_HighDensity_ReducedData(i).XdivPNEC(r,logical(row_acceptable_for_PNEC)) = S_HighDensity_ReducedData(i).XX(r,logical(row_acceptable_for_PNEC))./ cell2mat(S_HighDensity_ReducedData(i).avail_compounds_PNEC(logical(row_acceptable_for_PNEC)))';
        end
    end
end
